...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('admin.productos.show', $producto) }}" class="btn btn-outline-secondary btn-sm">← Volver</a>
</div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.productos.update', $producto) }}" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="id_categoria" class="form-select" required>
                        @foreach($categorias as $c)
                            <option value="{{ $c->id_categoria }}" {{ $producto->id_categoria == $c->id_categoria ? 'selected' : '' }}>{{ $c->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    <select name="id_proveedor" class="form-select">
                        ...
                        @foreach($proveedores as $p)
                            <option value="{{ $p->id_proveedor }}" {{ $producto->id_proveedor == $p->id_proveedor ? 'selected' : '' }}>{{ $p->nombre }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-6">
                    <label class="form-label fw-semibold">Agregar foto</label>
                ...
                <div class="col-md-6 d-flex align-items-end">
                    <div class="form-check">
                        <input type="checkbox" name="activo" class="form-check-input" id="activo" value="1" {{ $producto->activo ? 'checked' : '' }}>
